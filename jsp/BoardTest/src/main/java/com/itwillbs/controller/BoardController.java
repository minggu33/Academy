...
public class BoardController {
	...
	public String registPOST(BoardVO vo, RedirectAttributes rttr/*Model model*/) throws Exception{
		...
		// 입력정보를 전달받아서 DB에 정보를 저장 => 서비스 동작을 호출
	...
	public void readGET(@RequestParam("bno") int bno, Model model) throws Exception{
		...
		// * 전달되는 데이터 받을때 
		// @ModelAttribute : 컨트롤러가 뷰에 정보를 전달하기 위해서 사용 객체
		// => DTO/VO 객체를 주로 사용
		...
		// 서비스에 글 본문 내용을 가져오는 메서드
		BoardVO vo = service.read(bno);
		
		System.out.println("C :"+bno);
		...
		System.out.println("C : /board/read.jsp 페이지로 이동 ");
	...
	public void updateGET(@RequestParam("bno") int bno, Model model) throws Exception{
			...
			// 전달받은 정보 (글 번호)
			System.out.println("C : 수정할 글 번호 "+bno);
			
			// 서비스에 글 번호에 해당하는 글 정보를 가져오는 동작
			// model 객체에 저장			
			model.addAttribute("uvo",service.read(bno) );		
			
			// 페이지 이동(/board/modify.jsp)
			System.out.println("C : 정보 저장 후 페이지 이동 /board/modify.jsp");
