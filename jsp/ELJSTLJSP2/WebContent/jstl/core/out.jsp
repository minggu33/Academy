...
<body>
	...
	* JSP 표현식을 대신해서 사용
	...
	double result = 2*3;
	// JSP
		out.println("2*3 = "+(2*3)+"<br>");
	%>
	JSP 표현식 : 2*3=<%=2*3 %> <%=result %><br>
	core : <c:out value="2*3" /><br>	
	EL 표현식 : ${2*3}  ${result}<br>
	core+el : <c:out value="${2*3 }"/><br>
	core+JSP 표현식 : <c:out value="<%=2*3 %>"></c:out>
...
<%-- <%-- 	<%=member.getName() %> --%> 
	...
	<!-- core+el은 null값을 공백으로 출력 -> 이 부분만 사용 불가능. 사용자 입장에서 다른 기능은 사용할 수 있다. 추후 업데이트로 픽스가능-->
	<c:out value="${member.getName() }"/> <br>
	<!-- core+jsp는 null 에러.  페이지가 뜨지 않는다. -> 사용자 입장에서 다른기능도 다 사용할 수 없다. -->
	<%-- 	<c:out value="<%=member.getName() %> "/> <br> --%>
	
	<!-- 
		core+el 표현식으로 null 값을 공백으로 출력
			=> default 속성을 사용하면 원하는 값으로 null값을 대신해서 출력가능
	 -->
