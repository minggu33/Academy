/*
	생성자?
	 - 객체 생성시 딱 한번만 호출되는 메소드의 종류
  	 ...
  	 - 자바에서 객체 생성시 반드시 하나의 생성자는 호출되어야 한다.
...
class Number{
	...
	// 생성자1
	// - new Number(); 객체 생성시.. Number()생성자를 호출해 num변수의 값을 저장시킨다.
	// - 매개변수로 값을 전달받지 않고 num변수의 값을 10으로 초기화할 생성자
	public Number() {
		num = 10;
	...
	// 메소드
	// - 위 Number 객체 생성시 객체변수 num에 저장된 값을 반환하는 메소드
...
public class test60 {
	...
	// Number 클래스로부터 객체 생성시..Number()생성자를 호출해 num변수의 값 초기화
	...
	// Number 클래스로부터 객체 생성시 Number(int n)생성자를 호출할 때 30을 전달하여
	// num변수의 값을 초기화 시킴
