...
/*
	...
	2.자바의 메소드 종류 2가지
		...
		- Date클래스 내부에 getDate()메소드 : 현재 날짜 및 시간관련 데이터를 제공해 주는 메소드
		
		(2) 사용자 정의 메소드 : 우리 개발자가 특정 기능을 필요로 할 때마다 새롭게 기능을 추가해서 만드는 메소드
...
public class test47 {
	...
	//자바 프로그램의 시작점 main메소드
	// -> 메소드의 이름은 main이다.
	// -> 프로그램을 시작시키는 기능
	// -> main메소드 내부에서 다른 메소드를 호출해서 사용할 수 있다.
	// -> 같은 클래스 내부의 다른 메소드를 호출시 클래스명,메소드명 <--문법을 사용한다.
	// -> main메소드의 중괄호 { 열고  }닫는 기호 내에 존재하는 모든 문장들이 순차적으로 JVM에 의해 실행된다.
	public static void main(String[] args) {
	
		//main 메소드 내부에서 다른 메소드 호출 가능
		//첫번째)
		//System이라는 이름의 클래스 내부에
		//out이라는 상수에 저장된 PrintStream객체에 접근하여
		//PrintStream객체 내부에 ... println메소드가 만들어져 있으므로 메소드를 호출하여
		//그 기능을 사용할 수 있었다.
		System.out.println("안녕"); 
		// println(String value)메소드의 기능 : 매개변수 value로 전달받은 데이터 출력
