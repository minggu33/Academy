...
/*
		...
		배열 메모리 공간의 특징
			...
			4. 배열메모리의 저장된 데이터의 갯수(배열의 크기) : 배열명.length; 사용
			5. 배열메모리에 각 칸에 저장되어 있는 값을 가져와서 사용하는 방법
				(배열의 각 요소에 접근하는 방법)
				 : 인덱스 위치번호(첨자, 배열번호)를 사용
				 
			배열 메모의 주소값을 저장할 변수만들기(배열 선언),배열 메모리 생성,
			배열 메모리 만들기(배열 생성),
			배열 메모리에 값 저장(배열 초기화)
			하는 방법 2가지
			 1. new연산자를 이용하는 방법
...
public class test36 {
	...
	public static void main(String[] args) {
		...
		//배열메모리의 주소값을 저장할 변수만들기(배열 선언) 방법 : 자료명[] 변수명;
		// 1. 배열 선언
		  int []score; // int정수값들이 저장될 배열 메모리의 주소번지를 저장할 score변수 선언
		  
		  
		  //  메모리 만들기(배열생성)  방법 : 변수명 = new 자료형[배열크기];
		  // 2. 배열 생성
		  score = new int[5]; // 배열메모리에 int타입의 자료 5개를 저장할 배열 메모리 공간 생성
		  ...
		  // 배열 메모리에 각인덱스번호 위치에 저장되어 있는 값을 얻어 출력하는 방법 :
		  ...
		  //존재하지 않는 배열의 인덱스번호 위치에 접근하려고 할때 예외(에러)발생
		...
		// 5-1. for반복문을 이용하여 배열메모리에 저장된 데이터들을 반복해서 얻어 출력(length사용)
