/*
	...
	스레드를 사용하는 이유는?
	 - 동시에 여러가지 작업을 처리하고자 할때 사용하게 된다.
	 
	스레드 프로그램(클래스)을 만드는 방법?
		1. Thread클래스를 상속받아 스레드 프로그램(클래스)을 만드는 방법
		2. Runnable인터페이스의 추상메소드를 오버라이딩 받아 스레드 프로그램(클래스)을 만드는 방법
		


	주제 : 1. Thread클래스를 상속받아 스레드 프로그램(클래스)을 만드는 방법
...
public class ThreadEx01 extends Thread{
	...
	// 콜백 메소드란? 개발자가 호출하는 메소드가 아니라 거꾸로 시스템이 어느시점에 자동으로 호출해주는 메소드		
	// 작업스레드 객체들이 해야할 일을 구현하는 콜백 메소드
	@Override
	public void run() { // run메소드는 무조건 하나만 만든다
		...
		for(int i = 0; i<5; i++) {
			...
			// 작업스레드 2개중 현재 작업스레드에게 1초간 휴식 주기
			try {
			 // 현재 작업 중인 스레드 1초 대기
				sleep(1000);
