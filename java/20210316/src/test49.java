...
public class test49 {
	...
	// 방법2. 매개변수가 있고 리턴값이 없는 메소드 만들기
	
	// 메소드 이름 : sum
	// 기능 : 매개변수로 전달 받은 값까지의 자연수의 합구하는 기능
	
	
	public static void sum(int n) {
		// 매개변수란? sum이라는 메소드를 호출할때 전달 받은 값을 담아두기 위한 변수
		// 위 int n <- 매개변수 
	...
	// return 키워드
	// 메소드의 몸체부분이 실행된 후..
	// 연산결과 값을 가지고 메소드를 호출한 장소로 연산결과값을 되돌려 줄때 사용
	// 단 하나의 연산결과 값만 되돌려 줄 수 있다.
	// 메소드 이름 : abs
	// 기능 : 매개변수로 전달받은 값의 절대값을 구해 절대값을 되돌려주는 기능의 메소드
	
	public static int abs(int data) {
		//절대값 구하는 기능을 코드로 구현
		//매개변수 전달 받은 값이 음수일때 -> 양수로 변경
...
//		System.out.println(value);
		...
		// 1 ~ 5까지의 누적 합을 계산하여 출력하기 위해 sum(int n)메소드 호출!
		// 요약 : sum 메소드 호출시 5를 전달하여 1 ~ 5까지의 누적 합을 계산하여 출력
