...
// 멤버메소드
	...
	public static void setStaticNum(int staticNum) {
		// 클래스변수 staticNum에 매개변수로 전달 받은 데이터 저장
		// 방법
		// 클래스명.클래변수명 = 매개변수명;
...
public class test72 {
	...
	System.out.println(AAA.getStaticNum());	
		...
		/*
		1.인스턴스변수, 인스턴스 메소드 : 객체 생성시 객체 내부 메모리 존재.
		2. 클래스변수, 클래스메소드 : 클래스가 JVM메모리가 올라갈 때 존재
		3. 클래스메소드 내부에서 인스턴스 변수에 접근이 불가능하다.
		4. ! 경고메시지의 뜻 :
			정적인 클래스변수, 클래스 메소드는 정적인 방식으로 접근하라는 경고를 말함
			즉! 클래스명으로 접근하라는뜻.
