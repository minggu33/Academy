import java.util.Date; // java폴더 내부의 util폴더 내부의 Date.java 파일의 Date클래스를
					   // 현재 test67.java 내부에서 사용하기 위해
...
/*
	...
	- 동일한 이름의 메소드가 호출되더라도
	  전달인자의 자료형이나 개수로 해당 메소드를 구분해서 호출할 수 있기 때문이다.
...
public class test67 {
	...
	public static void main(String[] args) {
		...
		// -> 메소드호출시 전달 받는 인자의 자료형을 달리하여 메소드를 여러번 정의 해 놓았기 때문이다.
		
		// 자바 API Document문서에서
		// java.io.PrintStream.println메소드를 찾아 보자
		// println()
		// println(boolean x)
		// println(char x)
		// ...
		...
		// 1. java.lang 패키지는 자바에서 기본으로 제공되는 패키지 이기때문에
		//	  자동으로 사용하게 import(추가) 된다.
		//	  정확히 자바 컴파일러가 import문을 추가 해준다.
		// 2. System 클래스는 java.lang패키지에 속해 있는 클래스이다.
