...
/*
	...
	- 자바에서 문자열을 담아주는 클래스
	- String클래스를 이용해 String객체 메모리가 한번 생성된 다음 변하지 않는 문자열을 저장해주기 위해 사용.	
	- String객체 메모리에 문자열을 저장해 두면 변경되지 않음을 보장.
...
public class test118 {
	...
	public static void main(String[] args) {
		...
		//		   문자열 초기화를 위해 전달받는 생성자의 매개변수가 char배열타입의 변수로 선언
		//		   되어 있다.
		...
		// str1, str2 참조변수는 각각 동일한 String 객체 메모리를 사용할 수 있는지 비교
		if(str1 == str2) {
			System.out.println("서로 같은 String객체 메모리를 사용");
		}else {
			System.out.println("서로 다른 String객체 메모리를 사용");
		}
		
		// 방법1. 문자열 리터럴 상수를 이용하여 String객체 메모리 생성
		String str3 = "JSP";		// str3 참조변수
													// "JSP" 문자열객체 메모리 하나
		...
		/*
			...
			같은 문자열 상수 "JSP"데이터를 Stirng객체 메모리를 한번만 생성해서 저장하게 되며
		...
		*/
		...
		// String 객체 메모리에 저장된 전체문자열의 특정 인덱스 위치에 존재하는 문자 알아내는 메소드 호출
