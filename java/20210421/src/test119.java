...
public class test119 {
	...
	public static void main(String[] args) {
		...
		// String 클래스 내부에 만들어져있는 equals메소드, subString메소드
				// equals(Object o)메소드
				// : String 객체 메모리에 저장된 문자열데이터를 비교할 때 호출하는 메소드
		...
		// subString(int arg0)
		// : 지정한 인덱스 위치에서 마지막 위치까지의 문자열을 모두 잘라서 반환
		...
		// 7인덱스 위치에 존재하는 J부터 시작해서 10이전 인덱스 위치인 9인덱스 위치에 존재하는 
		// P 문자까지의 문자열 JSP를 잘라내어 반환
...
//		temp4 = c + d;
	...
	// String클래스 내부에 만들어져 있는 contains메소드, replace메소드
		...
		System.out.println("'해당문자열'이 포함되어 있는가? => "+e.contains("해당문자열"));
		...
		// "JAVAJSP" 전체 문자열 중에서 C문자만 !문자로 바꾼 후 바뀐 전체 "JAVAJSP!"문자열을 반환		
		System.out.println("문자단위로 변경->"+f.replace('C', '!'));
		// "JAVAJSP" 전체 문자열 중에서 "JSPC"문자열 부분만 "PROGRAMMING"로 바꾼 후
		// 바뀐 "JAVAPROGRAMMING" 문자열을 반환
		System.out.println("문자열단위로 변경->"+f.replace("JSPC", "PROGRAMMING"));
		
		// String클래스 내부에 만들어져 있는 그 밖의 메소드들
		
			// String toLowerCase메소드
			// -> String 객체메모시에 저장된 전체문자열의 모든 문자를 소문자로 바꿔 반환하는 메소드
		
			// String toUpperCase메소드
			// -> String 객체메모시에 저장된 전체문자열의 모든 문자를 대문자로 바꿔 반환하는 메소드
		
			// static String valueOf(Object obj)메소드
			// -> 매개변수로 전달받는 데이터를 무조건 문자열로 변환해서 반환하는 메소드
			...
			/*
				...
				request.getParameter("age"); -> 문자열 "100"
				
				100 + 1 연산시 "100"문자열을 숫자 100으로 변환해서 계산해야 한다.
				Integer.parseInt(request.getParameter("age")) -> 숫자 100으로 변환된다.
