...
/*
	...
	- 1Byte단위로 데이터를 읽어들이거나 출력하는게 아니라
	  ...
	  ByteArrayInputStream클래스의 생성자
	    ...
	    -> 매개변수로 전달받는 배열크기 단위로 데이터를 읽어들일 수 있다.
...
public class ByteInputOutputEx1 {
	...
	public static void main(String[] args) throws IOException {
		...
		while ((cnt = fin.read(buff)) != -1) { // 읽어올 데이터가 존재할때 반복
			...
			// 지금까지 한일 -> 파일에서 읽어들인 내용을 바이트배열 형태로
			// 				 ByteArrayOutputStream 메모리에 저장했다.
		
			// 바이트 배열 형태로 저장시킨 메모리 공간 -> ByteArrayOutputStream 메모리에서
			// byte[]배열 꺼내오기
