...
// 도스로 실행하는 방법
// 예) DOS명령어 중에서 Type프로그램을 통해서 텍스트 파일의 내용을 읽어들일 수 있음 (맥에서는 cat?)
// 방법
// 순서1) cmd 실행
// 순서2) A.txt파일이 존재하는 위치 D:로 이동한다.
// 순서3) type프로그램을 통해 A.txt 파일의 내용을 읽어들여 cmd 창에 출력한다.
// 		 소스코드 -> type 텍스트파일명.확장자
//				   type A.txt
...
public class JType {
	...
	public static void main(String[] args) throws IOException {
		...
		/*
		 	주 기억장치 : 프로그램은 실제 주기억장치에 존재한다.
		 				그러므로 실제 CPU는 주기억장치를 통해서 프로그램에 접근하여 사용함.
		 
		 	보조 기억장치(HDD,SSD) : 
		 	CPU가 사용할 수 없는 파일이 들어 있는 장치
		 	
		 	결론 : CPU는 보조기억장치에 직접 접근 할 수 없으므로
		 		  보조기억장치에 있는 파일을 주기억장치 메모리에 올려놓기(로드)위해
		 		  주기억장치 내부에 파일이 저장될 수 있는 메모리 공간 만들기
		 		  -> FileInputStream 클래스에 대한 객체메모리 생성
		  ...
		  2번. 명령 인자를 통해 불러올 수 있는 파일을 입력 받는 방법
		 */
		...
		do {
			i = fis.read(); // 한바이트 읽어들이기
			
			// 만약 읽어들일 데이터가 존재한다면?(i변수의 값이 -1과 같지 않다면)
			if(i != -1) {
				// 읽어들인 데이터를 바로 콘솔에 출력
				System.out.println((char)i);
				
			}
			// 파일로부터 읽어들일 데이터가 존재할때까지 반복
		}while(i != -1);
