...
// 프레임(창) 만들기 두가지 방법
// 방법 1. main()메소드내부에서 JFrame클래스에 대한 객체 생성
// 방법 2. JFrame클래스를 상속받는 사용자정의 프레임클래스를 만들어
//		  사용자정의 프레임클래스에 대한 객체를 main()메소드내부에서 생성


// 방법 1. main()메소드내부에서 JFrame클래스에 대한 객체 생성
//		순서 1) awt, swing 패키지 내부에 있는 전체화면을 구성하기 위한
// 			   모든 클래스나 인터페이스를 사용하겠다 라고 import 선언
import java.awt.*;
import javax.swing.*;


//			1-1) awt, swing 패키지 내부에 있는 이벤트 등록 관련한 클래스나 인터페이스를 사용하겠다라고 import 선언
...
public class MyApp {
	...
	public static void main(String[] args) {
		...
		// 순서 2) 프레임창 만들기
		//		  방법 : JFrame 클래스에 대한 객체 생성시 만들 창제목을 생성자로 전달하여 만들기
		...
		/*
		 	...
		 	생성된 JFrame객체는 3공간으로 구성됨
		 	1 공간 : Menu Bar가 들어가는 공간
		 	2 공간 : Frame들이 들어가는 공간
		 	3 공간 : Content Pane들이 들어가는 공간
		 	...
		 	ContentPane :
		 		...
		 		ContentPane 컨테이너객체를 얻어서 여러 버튼 컴포넌트를 부착해 봅시다.
		 				 
		 */
		
		// JFrame객체의 getContentPane() 메소드를 호출하여 반환데이터로
		// ContentPane 컨테이너 객체를 반환받아 얻어올 수 있다.
		// (반환값으로 Container부모클래스타입으로 자식 ContentPane컨테이너객체를 반환 받는다)
		Container contentPane = f.getContentPane();
		
		// JFrame창 내부에 존재하는 ContentPane컨테이너객체에 JButton컴포넌트를 생성해서 부착
		...
		// 새로운 JPanel컨테이너를 만들어서 붙이기
		MyPanel mp = new MyPanel();
		
		// JFrame창에 새로운 JPanel(new MyPanel();) 부착 (붙이기)
		f.setContentPane(mp);
		
		// 위 Click버튼을 다시 JPanel(new MyPanel();) 부착
		mp.add(b1);
...
/*
결론	1 : 방법1은 단순하지만 권장되지 않는다.
		프레임창에 스윙 컴포넌트들을 추가하는 과정이 불편하며
		확장성, 융통성이 결여되는 단점이 있기 때문
결론	2 : JDK 1.5버전 부터는 JFrame을 수정하여
		개발자가 내부적으로 JFrame의 add()메소드는 추가되는 컴포넌트를
		자동으로 ContentPane에 부착하도록 하고 있음
		따라서 개발자들은 ContentPane을 의식하지 않고
