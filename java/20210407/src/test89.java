...
/*
	...
	1. 부모클래스의 멤버와 자식클래스의 멤버가 같은 이름을 가질경우
		-> 부모클래스의 멤버를 사용할 때 "super.부모멤버이름" 사용
	
	2. 자식클래스에서 명시적으로 부모클래스의 생성자를 호출할 경우
		-> super(매개변수) 형식으로 사용한다.
		-> 주의점 : 반드시 자식클래스의 생성자를 정의할 때
				  super(매개변수)는 첫 번째 행에서 부모생성자를 호출 해야한다.

*/

class Person{ // 사람클래스 -> 부모클래스 역할
...
class Man extends Person{
	...
	int age = 40; // Person 부모클래스의 인스턴스변수 age와
				  // Man 자식클래스의 인스턴스변수 age 이름이 같다.
	...
	public Man(String name) {
		...
		// -> Man(자식)객체 생성시.. 자식객체의 생성자가 호출되어서
		...
		// -> 인스턴스 변수 name이 Man클래스가 아닌 Person(부모)클래스에 선언된 인스턴스변수이기 때문에
		// 	  변수가 선언된 곳에서 변수초기화 작업을 해줘야 하기때문이다.
	}
...
public class test89 {
	...
	public static void main(String[] args) {
		...
		// 자식 Man 객체 생성시.. 생성자를 호출해 "홍길동" 전달 후 변수값 초기화
		Man m = new Man("홍길동");
