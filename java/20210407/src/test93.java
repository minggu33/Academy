...
/*
 	...
 	객체 지향 개념 자바에서의 다형성 : 
 		-> 하나의 클래스 타입의 참조변수로 여러클래스타입으로 생성 객체들을 참조할 수 있도록 하는 것.
...
//			t1 = new CaptionTV(); // 자식객체3 <-- 업캐스팅이 일어남(다형성특징)
	// 부모클래스 TV의 참조변수 t1은 하나다
		...
		// 1. 의 경우 c1 참조변수로 참조할 수 있는 멤버에 접근해보자.
		c1.power = true;
		c1.channel = 0;
		c1.power();
		c1.channelUp();
		...
		// 2. 번의 경우 tv2참조변수로 참조할 수 있는 멤버 접근해보자
...
/*
			실제 생성한 객체 메모리가 new CaptionTV()객체 일지라도
			참조변수 tv2로는 new CaptionTV()객체 메모리에 저장된 모든 멤버를 사용할 수 없다.
...
*/
		...
		// 이유 : 실제 TV객체 멤버의 개수보다 CaptionTV클래스 타입의 참조변수 c2가 사용할 수 있는 
		//		 멤버의 개수가 더 많기 때문이다.
		// 이유요약 : CaptionTV클래스 타입의 c2참조변수가 참조하고 있는 객체는 TV객체이고
		...
		/*
			...
			- 참조변수가 사용할 수 있는 멤버의 개수는 객체메모리의 멤버 개수보다 같거나 작아야한다.
