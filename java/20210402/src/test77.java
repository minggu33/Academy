...
class Product2{
	...
	// 생성된 객체(인스턴스)의 갯수를 저장하기 위한 클래스 변수 count선언
	static int count = 0; // 1위
	
	// 객체(인스턴스)의 고유번호를 저장하기 위한 인스턴스변수 serialNo선언
	int serialNo; // 3위
	
	// 객체(인스턴스) 변수의 값을 저장시키는
	// 인스턴스 초기화블럭 사용
	// 사용용도 : new Product2() 객체가 생성될 때마다 
	// 위 count변수의 값을 1씩 증가시켜서 serialNo변수에 저장
	{
		++ count;
		serialNo = count;
	} // 4위
